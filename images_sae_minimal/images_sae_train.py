...
def main():
    ...
    # 创建图像目录
    image_dir = "../output_images_jpg_rename"  # 调整目录路径以访问原始图像
    
    # 检查图像目录是否存在
    if not os.path.exists(image_dir):
        print(f"错误: 图像目录 {image_dir} 不存在。请确保数据目录位置正确。")
        return
            ...
            for sae_id in range(config['num_saes']):
                ...
                if epoch + 1 == config['num_epochs']:  # 只在最后一个epoch可视化
                    for i, example in enumerate(examples[:5]):  # 限制为5个示例
                        ...
                        # 保存图像到本地
                        local_path = f"sae{sae_id}_epoch{epoch+1}_ex{i}.png"
